...
export async function handleQuestionImageExtraction(req, res) {
    ...
    const groqBaseUrl = (process.env.AI_BASE_URL || "https://api.groq.com/openai/v1").replace(/\/$/, "");
    
    const geminiApiKey = process.env.GEMINI_API_KEY;
    const geminiBaseUrl = "https://generativelanguage.googleapis.com/v1beta/openai";

    if (!geminiApiKey || !groqApiKey) {
        sendJson(res, 501, { error: "GEMINI_API_KEY or AI_API_KEY is not configured" });
        return;
    }
    ...
    // Vision Models Pipeline (Fallback Mechanism)
    const visionModels = [
        { provider: "gemini", model: "gemini-2.5-flash" }, // Primary
        { provider: "groq", model: process.env.AI_VISION_MODEL || "llama-3.2-90b-vision-preview" } // Fallback
    ];
    ...
    if (!firstContent) {
        sendJson(res, 500, { error: visionError || "All vision models failed" });
        return;
    }

    // Text Models Pipeline (Fallback Mechanism)
    const textModels = [
        { provider: "groq", model: process.env.AI_MODEL || "llama-3.3-70b-versatile" }, // Primary
        { provider: "groq", model: "llama-3.1-8b-instant" }, // Emergency Third AI from Groq
        { provider: "gemini", model: "gemini-2.5-flash" } // Final Fallback
    ];
    ...
    if (!finalContent) {
        sendJson(res, 500, { error: textError || "All formatting models failed" });
        return;
    }

    sendJson(res, 200, parseExtractedQuestion(finalContent));
}
