...
export class Toolbar extends BaseComponent {
  ...
  render() {
    ...
    if (selectedCount > 0) {
      actionsHtml = `
        <div class="flex items-center gap-4 bg-primary/10 text-primary px-4 py-2 rounded-lg border border-primary/20 animate-fade-in">
          ...
          <button id="clear-selection-btn" class="hover:text-white transition-colors ml-2" title="إلغاء التحديد"><i data-lucide="x" class="w-5 h-5"></i></button>
        </div>
      `;
    } else {
      actionsHtml = `
        <div class="flex items-center gap-2">
          ${this.onGenerateAI ? `
            <button id="generate-ai-btn" class="btn bg-purple-600/20 text-purple-400 hover:bg-purple-600 hover:text-white border border-purple-500/30 transition-colors">
              <i data-lucide="sparkles" class="w-4 h-4 ml-2"></i> توليد بالذكاء الاصطناعي
            </button>
          ` : ''}
          <button id="add-btn" class="btn btn-primary">
            <i data-lucide="plus" class="w-5 h-5 ml-2"></i> إضافة سؤال
          </button>
        </div>
      `;
    }
    ...
    this.element.innerHTML = `
      ...
      <div id="toolbar-sort" class="mr-4"></div>
    `;
