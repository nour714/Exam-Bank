...
async function handleAiMentor(req, res) {
    ...
    const mentorResponse = parseMentorResponse(content, subject, message);
    sendJson(res, 200, mentorResponse);
}

async function handleQuestionImageExtraction(req, res) {
    const apiKey = process.env.AI_API_KEY;
...
async function handleGenerateSimilar(req, res) {
    const apiKey = process.env.AI_API_KEY;
    const baseUrl = (process.env.AI_BASE_URL || "https://generativelanguage.googleapis.com/v1beta/openai").replace(/\/$/, "");
    const model = process.env.AI_MODEL || "gemini-2.5-flash";
    if (!apiKey) { sendJson(res, 501, { error: "AI_API_KEY is not configured" }); return; }
    const body = await readJsonBody(req);
    ...
    if (!questionText) { sendJson(res, 400, { error: "questionText is required" }); return; }
    ...
    const data = await providerResponse.json().catch(() => ({}));
    if (!providerResponse.ok) {
        sendJson(res, providerResponse.status, { error: data.error?.message || "AI provider request failed" });
        return;
    }
    const content = data.choices?.[0]?.message?.content || "";
